...
def plot_damage_fallof(hero_stats):
    ...
    # Set the crit rate at 0.0 and hold constant for this script
    crit_accuracy = 0.0
    # Iterate over the two different falloff values, 0.3 being the new value, and 0.5 being the value pre-patch
    for falloff_max in [0.3, 0.5]:

        # Set a color per falloff rate so that the color is consistent across plots
        if falloff_max == 0.3:
            color = 'r'
        if falloff_max == 0.5:
            color = 'g'
        ...
        # Plot DPS/HPS
        plt.figure(1)
        plt.plot(x, y, label='{0} DPS ({1} falloff)'.format(hero_stats[4], falloff_max), color=color)
        ...
        plt.plot(x_ttk, y_mercy_ttk, label='TTK with Mercy ({} falloff)'.format(falloff_max), color=color)
        plt.plot(x_ttk, y_ttk, label='TTK with out Mercy ({} falloff)'.format(falloff_max), color=color, linestyle='dashed')

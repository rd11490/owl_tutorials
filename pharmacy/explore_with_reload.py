...
def true_ttk(hero_stats):
    ...
    for falloff in [0.3, 0.5]:
        ...
        for cr in [0.0, 0.5, 1.0]:
            ...
            if max_dist > 0:
                print(
                    'At a distance of {0} Meters, crit rate of {1}, and a falloff of {2}, {3} can kill a Pharah with a mercy pocket after {4} seconds'.format(
                        max_dist, cr, falloff, hero_stats[6], round(float(ttk_end) / tick_rate, 3)))
            plt.plot(x, y, linestyle=linestyle, color=color,
                     label='{0} ({1} % crit rate, {2} falloff)'.format(hero_stats[6], cr * 100, falloff))

...
for ind in pb_stats.index:
    ...
    team = row['team_name']
    color = Teams.TeamColors[team]
    plt.text(attach_rate + 0.004, kill_rate + 0.004, label, color=color, weight='bold')


x_label = pb_stats['Attach Rate'].max() - 0.05
y_label = pb_stats['Kill Rate'].min() + 0.005
plt.text(x_label, y_label, '*Dot size based on \nnumber of pulse bombs used', weight='bold', size='x-small')

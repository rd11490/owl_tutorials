...
# Set a maximum schedule difference we will allow for. By setting it to 15, we are saying that we desire no schedule to be
# +- 15 points from the average
max_sos_difference = 12
...
team_schedules = team_schedules.merge(rankings, left_on='opponentShortName', right_on='team')
teams = list(team_schedules['teamName'].unique())

team_schedules = team_schedules[['teamName', 'opponentName', 'rank']]
team_schedules = team_schedules.rename(columns={'rank': 'opponentRank'})
...
teams_and_ratings = team_schedules[['opponentName', 'opponentRank']].drop_duplicates()
team_rankings = {}
for i in teams_and_ratings.index:
    row = teams_and_ratings.loc[i, :]
    team_rankings[row['opponentName']] = row['opponentRank']
...
def build_second_half_schedule_and_counter():
    team_second_half = {}
    team_counter = {}
    for t in teams:
        team_second_half[t] = []
...
def initialize_schedule():
    team_second_half, team_counter = build_second_half_schedule_and_counter()
...
is_balanced = False
team_second_half, team_counter = initialize_schedule()
...
def calc_sos_diff_for_swap_set(swap_set):
    set1, set2 = swap_set
...
def smart_swap_teams(swap_team1, swap_team1_schedule, swap_team2, swap_team2_schedule):
    ...
    if len(combinations) == 0:
        return None, None
    min_swap_set = min(combinations, key=lambda x: calc_sos_diff_for_swap_set(x))
...
def get_random_schedules_to_swap_smart(team_sos):
    swap_set = False
    while not swap_set:
        swap_team1, swap_team2 = find_randoms_schedules(team_sos)
        swap_team1_schedule = team_second_half[swap_team1]
        swap_team2_schedule = team_second_half[swap_team2]
        to_swap_team_1, to_swap_team_2 = smart_swap_teams(swap_team1, swap_team1_schedule, swap_team2, swap_team2_schedule)
...
while not is_balanced:
    ...
    is_balanced, sched_sos = check_balanced(team_sos)
    ...
    # take top and bottom
    # swap 1 random team each (make sure not to have a team play themself
team_sos = {}
for k, v in best_schedule.items():
    team_sos[k] = sum([i[1] for i in v])

check_balanced(team_sos)
double_schedule = []
for k, v in best_schedule.items():
    print(f'SOS {k}: {v}, {sum([i[1] for i in v])}')
    double_schedule.append({
